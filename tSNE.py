#%%
import numpy as np
from sklearn.svm import SVC
from sklearn.model_selection import train_test_split, cross_val_score
from sklearn.metrics import accuracy_score,precision_score,confusion_matrix,classification_report, precision_recall_curve
import pandas as pd
import matplotlib.pyplot as plt
from sklearn.decomposition import PCA
from sklearn.manifold import TSNE
import time

train_image = np.fromfile('train-images-idx3-ubyte', dtype = np.uint8)
train_image = np.delete(train_image, range(16))
train_image = train_image.reshape(-1, 784)
print(train_image.shape)

train_label = np.fromfile('train-labels-idx1-ubyte',dtype = np.uint8)
train_label = np.delete(train_label, range(8))
print(train_label.shape)

test_image = np.fromfile('t10k-images-idx3-ubyte',dtype = np.uint8)
test_image = np.delete(test_image, range(16))
test_image = test_image.reshape(-1, 784)
print(test_image.shape)

test_label = np.fromfile('t10k-labels-idx1-ubyte',dtype = np.uint8)
test_label = np.delete(test_label, range(8))
print(test_label.shape)


classes = ['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat', 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']
features = ['pixel' + str(i+1) for i in range(test_image.shape[1]) ]
tsne_df = pd.DataFrame(test_image, columns=features)
tsne_df['label'] = test_label

results = []
# Loop through all label
for i in range(tsne_df.shape[0]):
    # Extract the label for comparison
    if tsne_df['label'][i] == 0:
        # Save meaningful label to the results
        results.append('T-shirt/top')
    # Following the same code pattern as the one above
    elif tsne_df['label'][i] == 1:
        results.append('Trouser')
    elif tsne_df['label'][i] == 2:
        results.append('Pullover')
    elif tsne_df['label'][i] == 3:
        results.append('Dress')
    elif tsne_df['label'][i] == 4:
        results.append('Coat')
    elif tsne_df['label'][i] == 5:
        results.append('Sandal')
    elif tsne_df['label'][i] == 6:
        results.append('Shirt')
    elif tsne_df['label'][i] == 7:
        results.append('Sneaker')
    elif tsne_df['label'][i] == 8:
        results.append('Bag')
    elif tsne_df['label'][i] == 9:
        results.append('Ankle boot')
    else:
        print("The dataset contains an unexpected label {}".format(tsne_df['label'][i]))

# Create a new column named result which has all meaningful results        

rand = np.random.permutation(tsne_df.shape[0])
N = 10000
tsne_df_subset = tsne_df.loc[rand[:N],:].copy()
data_subset = tsne_df_subset[features].values

time_start = time.time()
tsne = TSNE(n_components=2, verbose=1, perplexity=40, n_iter=300)
tsne_results = tsne.fit_transform(test_image)
print('t-SNE finished! Time elapsed: {} seconds'.format(time.time()-time_start))

tsne_df_subset['t-SNE First Dimension'] = tsne_results[:,0]
tsne_df_subset['t-SNE Second Dimension'] = tsne_results[:,1]
plt.figure(figsize=(16,10))
plt.scatter(tsne_results[:,0], tsne_results[:,1], c = test_label, s = 0.5)
# %%
